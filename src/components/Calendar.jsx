    ...
    return (
        <div style={{ fontFamily: "'Inter', sans-serif" }}>
            ...
            {showAddModal && (
                ...
                >
                    <div onClick={e => e.stopPropagation()} style={{ ...glass, width: '100%', maxWidth: '420px', padding: '28px', margin: '0 16px', animation: 'popIn 0.18s ease' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px', flexWrap: 'wrap', gap: '12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '3px' }}>
                        {['month', 'week'].map(v => (
                            <button key={v} onClick={() => setView(v)} style={{ padding: '6px 14px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 700, background: view === v ? 'rgba(99,102,241,0.3)' : 'transparent', color: view === v ? '#c7d2fe' : '#64748b', textTransform: 'capitalize', transition: 'all 0.15s' }}>
                                {v}
                            </button>
                        ))}
                    </div>
                    {/* Month nav */}
                    <button onClick={() => setCurrentMonth(m => subMonths(m, 1))} style={{ width: '38px', height: '38px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.04)', color: '#94a3b8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <ChevronLeft size={18} />
                    </button>
                    <span style={{ fontSize: '16px', fontWeight: 700, color: '#fff', minWidth: '140px', textAlign: 'center' }}>{format(currentMonth, 'MMMM yyyy')}</span>
                    <button onClick={() => setCurrentMonth(m => addMonths(m, 1))} style={{ width: '38px', height: '38px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.04)', color: '#94a3b8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <ChevronRight size={18} />
                    </button>
                    <button onClick={() => { setSelectedDate(new Date()); setShowAddModal(true); }} style={btnPrimary}>
                                ...
                                {calendarDays.map((day, i) => {
                                    ...
                                    return (
                                        <div key={i} onClick={() => setSelectedDate(day)}
                                            style={{
                                                minHeight: '100px', padding: '8px 6px',
