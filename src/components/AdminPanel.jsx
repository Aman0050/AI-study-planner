        ...
        <div style={{ fontFamily: "'Inter', sans-serif" }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'linear-gradient(135deg,#ef4444,#f97316)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 20px rgba(239,68,68,0.3)', flexShrink: 0 }}>
                        <ShieldAlert color="white" size={24} />
                    </div>
                    <div>
                        <h1 style={{ fontSize: window.innerWidth < 480 ? '22px' : '26px', fontWeight: 800, color: '#fff', margin: 0, fontFamily: "'Outfit', sans-serif" }}>Admin Dashboard</h1>
                        <p style={{ color: '#64748b', margin: '3px 0 0', fontSize: '13px' }}>Manage users and analytics</p>
                    </div>
                </div>
                <button onClick={loadData} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#94a3b8', borderRadius: '10px', padding: '10px 16px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', width: window.innerWidth < 480 ? '100%' : 'auto', justifyContent: 'center' }}>
            ...
            {loading && !stats ? (
                ...
                <>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px', marginBottom: '24px' }}>
                        {[
                            { icon: Users, label: 'Users', value: stats?.totalUsers || 0, color: '#3b82f6', bg: 'rgba(59,130,246,0.12)' },
                            { icon: Shield, label: 'New (7d)', value: `+${stats?.newUsers || 0}`, color: '#10b981', bg: 'rgba(16,185,129,0.12)' },
                            { icon: FileText, label: 'Tasks', value: stats?.totalTasks || 0, color: '#8b5cf6', bg: 'rgba(139,92,246,0.12)' },
                            { icon: CheckCircle2, label: 'Done', value: stats?.completedTasks || 0, color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' },
                        ].map(s => (
                            <div key={s.label} style={{ ...glass, padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
